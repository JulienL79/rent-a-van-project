...
import { NewVehicle } from "../entities/index.js";
...
export const vehiclesModel = {
  ...
  getAllByCategory: async (id: string) => {
    try {
      return await db.query.vehicles.findMany({
  ...
  attachEquipments: async (vehicleId: string, equipmentIds: string[]) => {
    try {
  ...
  getEquipments: async (vehicleId: string) => {
    try {
  ...
  detachEquipments: async (vehicleId: string, equipmentIds: string[]) => {
    try {
      await db
